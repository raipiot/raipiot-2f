import type { OmitCurrentAndSize, SampleSheetsPageDto, SampleSheetVo } from '@raipiot-2f/api'
...
export function CreatePage() {
  ...
  // Delete mutation
  const { mutateAsync, isPending: isRemovePending } = SampleSheets.useRemoveMutation()
  // Publish mutation
  const { mutateAsync: publishMutateAsync, isPending: isPublishPending } =
    SampleSheets.usePublishMutation()
  ...
  const {
    data: { records, total },
    refetch
  } = useSuspenseQuery(SampleSheets.queries.listOP(PageUtils.mergeParams(pageParams)))
      ...
      <RpSearchBar
        ...
        onPrefetch={(values) =>
          queryClient.prefetchQuery(
            SampleSheets.queries.listOP(PageUtils.mergeParams(pageParams, values))
          )
        }
      />
