import type { PostPageDto, PostVo } from '@raipiot-2f/api'
...
function PostTable() {
  ...
  // 多选器：范型为列表行数据类型
  const { rowSelection, clearSelectedRowKeys } = useRowSelection<PostVo>()
  // 弹窗
  const modal = useModal()
  // 搜索表单
  const { searchForm, searchFormItems } = usePostsSearchForm()
  // 弹窗表单
  const { modalForm, modalFormItems } = usePostsModalForm()
  // 表格列
  const { columns } = usePostsColumns({ modal, form: modalForm })

  // 异步查询：列表数据
  const {
    data: { records, total },
    refetch
  } = useSuspenseQuery(postsQueryOptions(PageUtils.mergeParams(pageParams)))
  // 异步删除
  const { mutateAsync: removeMutateAsync, isPending: isRemovePending } = usePostRemoveMutation()
  // 异步提交
  const { mutateAsync: submitMutateAsync, isPending: isSubmitPending } = usePostSubmitMutation()
  ...
  return (
    ...
    >
      ...
      <RpSearchBar
        ...
        // 事件：预渲染
        onPrefetch={(values) =>
          queryClient.prefetchQuery(postsQueryOptions(PageUtils.mergeParams(pageParams, values)))
        }
      />
      {/* 表格 */}
      <RpBasicTable<PostVo>
        ...
        pagination={pagination({
          total,
          // 事件：分页预渲染
          onPrefetch: (values) => queryClient.prefetchQuery(postsQueryOptions(values))
        })}
        // 刷新加载
